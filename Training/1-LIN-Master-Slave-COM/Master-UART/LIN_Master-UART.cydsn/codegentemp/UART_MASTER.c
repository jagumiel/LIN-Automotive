...
static void UART_MASTER_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void UART_MASTER_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            UART_MASTER_RX_SDA_MOSI_HSIOM_SEL_GPIO,
            UART_MASTER_TX_SCL_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (UART_MASTER_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[UART_MASTER_RX_SDA_MOSI_PIN_INDEX] = UART_MASTER_RX_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [UART_MASTER_RX_SDA_MOSI_PIN_INDEX] = UART_MASTER_PIN_DM_OD_LO;
        #elif (UART_MASTER_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[UART_MASTER_RX_WAKE_SDA_MOSI_PIN_INDEX] = UART_MASTER_RX_WAKE_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [UART_MASTER_RX_WAKE_SDA_MOSI_PIN_INDEX] = UART_MASTER_PIN_DM_OD_LO;
        #else
        #endif /* (UART_MASTER_RX_SDA_MOSI_PIN) */
        
        #if (UART_MASTER_TX_SCL_MISO_PIN)
            hsiomSel[UART_MASTER_TX_SCL_MISO_PIN_INDEX] = UART_MASTER_TX_SCL_MISO_HSIOM_SEL_I2C;
            pinsDm  [UART_MASTER_TX_SCL_MISO_PIN_INDEX] = UART_MASTER_PIN_DM_OD_LO;
        ...
        {
        #if (UART_MASTER_RX_SDA_MOSI_PIN)
            hsiomSel[UART_MASTER_RX_SDA_MOSI_PIN_INDEX] = UART_MASTER_RX_SDA_MOSI_HSIOM_SEL_SPI;
        #elif (UART_MASTER_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[UART_MASTER_RX_WAKE_SDA_MOSI_PIN_INDEX] = UART_MASTER_RX_WAKE_SDA_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (UART_MASTER_RX_SDA_MOSI_PIN) */
        
        #if (UART_MASTER_TX_SCL_MISO_PIN)
            hsiomSel[UART_MASTER_TX_SCL_MISO_PIN_INDEX] = UART_MASTER_TX_SCL_MISO_HSIOM_SEL_SPI;
        #endif /* (UART_MASTER_TX_SCL_MISO_PIN) */
        
        #if (UART_MASTER_CTS_SCLK_PIN)
            hsiomSel[UART_MASTER_CTS_SCLK_PIN_INDEX] = UART_MASTER_CTS_SCLK_HSIOM_SEL_SPI;
        #endif /* (UART_MASTER_CTS_SCLK_PIN) */

            if (UART_MASTER_SPI_SLAVE == subMode)
            {
                /* Slave */
                pinsDm[UART_MASTER_RX_SDA_MOSI_PIN_INDEX] = UART_MASTER_PIN_DM_DIG_HIZ;
                pinsDm[UART_MASTER_TX_SCL_MISO_PIN_INDEX] = UART_MASTER_PIN_DM_STRONG;
                pinsDm[UART_MASTER_CTS_SCLK_PIN_INDEX] = UART_MASTER_PIN_DM_DIG_HIZ;

            #if (UART_MASTER_RTS_SS0_PIN)
                /* Only SS0 is valid choice for Slave */
                hsiomSel[UART_MASTER_RTS_SS0_PIN_INDEX] = UART_MASTER_RTS_SS0_HSIOM_SEL_SPI;
                pinsDm  [UART_MASTER_RTS_SS0_PIN_INDEX] = UART_MASTER_PIN_DM_DIG_HIZ;
            #endif /* (UART_MASTER_RTS_SS0_PIN) */

            #if (UART_MASTER_TX_SCL_MISO_PIN)
                /* Disable input buffer */
                 pinsInBuf |= UART_MASTER_TX_SCL_MISO_PIN_MASK;
            #endif /* (UART_MASTER_TX_SCL_MISO_PIN) */
            }
            else 
            {
                /* (Master) */
                pinsDm[UART_MASTER_RX_SDA_MOSI_PIN_INDEX] = UART_MASTER_PIN_DM_STRONG;
                pinsDm[UART_MASTER_TX_SCL_MISO_PIN_INDEX] = UART_MASTER_PIN_DM_DIG_HIZ;
                pinsDm[UART_MASTER_CTS_SCLK_PIN_INDEX] = UART_MASTER_PIN_DM_STRONG;

            #if (UART_MASTER_RTS_SS0_PIN)
                hsiomSel [UART_MASTER_RTS_SS0_PIN_INDEX] = UART_MASTER_RTS_SS0_HSIOM_SEL_SPI;
                pinsDm   [UART_MASTER_RTS_SS0_PIN_INDEX] = UART_MASTER_PIN_DM_STRONG;
                pinsInBuf |= UART_MASTER_RTS_SS0_PIN_MASK;
            #endif /* (UART_MASTER_RTS_SS0_PIN) */
            ...
            #if (UART_MASTER_RX_SDA_MOSI_PIN)
                pinsInBuf |= UART_MASTER_RX_SDA_MOSI_PIN_MASK;
            #elif (UART_MASTER_RX_WAKE_SDA_MOSI_PIN)
                pinsInBuf |= UART_MASTER_RX_WAKE_SDA_MOSI_PIN_MASK;
            #else
            #endif /* (UART_MASTER_RX_SDA_MOSI_PIN) */

            #if (UART_MASTER_CTS_SCLK_PIN)
                pinsInBuf |= UART_MASTER_CTS_SCLK_PIN_MASK;
            #endif /* (UART_MASTER_CTS_SCLK_PIN) */
            }
        }
        else /* UART */
        {
            if (UART_MASTER_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (UART_MASTER_TX_SCL_MISO_PIN)
                hsiomSel[UART_MASTER_TX_SCL_MISO_PIN_INDEX] = UART_MASTER_TX_SCL_MISO_HSIOM_SEL_UART;
                pinsDm  [UART_MASTER_TX_SCL_MISO_PIN_INDEX] = UART_MASTER_PIN_DM_OD_LO;
            #endif /* (UART_MASTER_TX_SCL_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (UART_MASTER_RX_SDA_MOSI_PIN)
                    hsiomSel[UART_MASTER_RX_SDA_MOSI_PIN_INDEX] = UART_MASTER_RX_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [UART_MASTER_RX_SDA_MOSI_PIN_INDEX] = UART_MASTER_PIN_DM_DIG_HIZ;
                #elif (UART_MASTER_RX_WAKE_SDA_MOSI_PIN)
                    hsiomSel[UART_MASTER_RX_WAKE_SDA_MOSI_PIN_INDEX] = UART_MASTER_RX_WAKE_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [UART_MASTER_RX_WAKE_SDA_MOSI_PIN_INDEX] = UART_MASTER_PIN_DM_DIG_HIZ;
                #else
                #endif /* (UART_MASTER_RX_SDA_MOSI_PIN) */
                }
                ...
                if (UART_MASTER_UART_MODE_STD == subMode)
                {
                    ...
                    #if (UART_MASTER_CTS_SCLK_PIN)
                        hsiomSel[UART_MASTER_CTS_SCLK_PIN_INDEX] = UART_MASTER_CTS_SCLK_HSIOM_SEL_UART;
                        pinsDm  [UART_MASTER_CTS_SCLK_PIN_INDEX] = UART_MASTER_PIN_DM_DIG_HIZ;
                    #endif /* (UART_MASTER_CTS_SCLK_PIN) */
                    }
                    ...
                    #if (UART_MASTER_RTS_SS0_PIN)
                        hsiomSel[UART_MASTER_RTS_SS0_PIN_INDEX] = UART_MASTER_RTS_SS0_HSIOM_SEL_UART;
                        pinsDm  [UART_MASTER_RTS_SS0_PIN_INDEX] = UART_MASTER_PIN_DM_STRONG;
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        UART_MASTER_SET_HSIOM_SEL(UART_MASTER_RX_SDA_MOSI_HSIOM_REG,
                                       ...
                                       UART_MASTER_RX_SDA_MOSI_HSIOM_POS,
                                        hsiomSel[UART_MASTER_RX_SDA_MOSI_PIN_INDEX]);

        UART_MASTER_uart_rx_i2c_sda_spi_mosi_SetDriveMode((uint8) pinsDm[UART_MASTER_RX_SDA_MOSI_PIN_INDEX]);

        #if (!UART_MASTER_CY_SCBIP_V1)
            UART_MASTER_SET_INP_DIS(UART_MASTER_uart_rx_i2c_sda_spi_mosi_INP_DIS,
                                         UART_MASTER_uart_rx_i2c_sda_spi_mosi_MASK,
                                         (0u != (pinsInBuf & UART_MASTER_RX_SDA_MOSI_PIN_MASK)));
        #endif /* (!UART_MASTER_CY_SCBIP_V1) */
        ...
        UART_MASTER_SET_HSIOM_SEL(UART_MASTER_RX_WAKE_SDA_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[UART_MASTER_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        UART_MASTER_uart_rx_wake_i2c_sda_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[UART_MASTER_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        UART_MASTER_SET_INP_DIS(UART_MASTER_uart_rx_wake_i2c_sda_spi_mosi_INP_DIS,
                                     UART_MASTER_uart_rx_wake_i2c_sda_spi_mosi_MASK,
                                     (0u != (pinsInBuf & UART_MASTER_RX_WAKE_SDA_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        UART_MASTER_SET_INCFG_TYPE(UART_MASTER_RX_WAKE_SDA_MOSI_INTCFG_REG,
                                        UART_MASTER_RX_WAKE_SDA_MOSI_INTCFG_TYPE_MASK,
    ...
    #if (UART_MASTER_TX_SCL_MISO_PIN)
        UART_MASTER_SET_HSIOM_SEL(UART_MASTER_TX_SCL_MISO_HSIOM_REG,
                                       UART_MASTER_TX_SCL_MISO_HSIOM_MASK,
                                       UART_MASTER_TX_SCL_MISO_HSIOM_POS,
                                        hsiomSel[UART_MASTER_TX_SCL_MISO_PIN_INDEX]);

        UART_MASTER_uart_tx_i2c_scl_spi_miso_SetDriveMode((uint8) pinsDm[UART_MASTER_TX_SCL_MISO_PIN_INDEX]);

    #if (!UART_MASTER_CY_SCBIP_V1)
        UART_MASTER_SET_INP_DIS(UART_MASTER_uart_tx_i2c_scl_spi_miso_INP_DIS,
                                     UART_MASTER_uart_tx_i2c_scl_spi_miso_MASK,
                                    (0u != (pinsInBuf & UART_MASTER_TX_SCL_MISO_PIN_MASK)));
    #endif /* (!UART_MASTER_CY_SCBIP_V1) */
    #endif /* (UART_MASTER_RX_SDA_MOSI_PIN) */

    #if (UART_MASTER_CTS_SCLK_PIN)
        UART_MASTER_SET_HSIOM_SEL(UART_MASTER_CTS_SCLK_HSIOM_REG,
                                       UART_MASTER_CTS_SCLK_HSIOM_MASK,
                                       UART_MASTER_CTS_SCLK_HSIOM_POS,
                                       hsiomSel[UART_MASTER_CTS_SCLK_PIN_INDEX]);

        UART_MASTER_uart_cts_spi_sclk_SetDriveMode((uint8) pinsDm[UART_MASTER_CTS_SCLK_PIN_INDEX]);

        UART_MASTER_SET_INP_DIS(UART_MASTER_uart_cts_spi_sclk_INP_DIS,
                                     UART_MASTER_uart_cts_spi_sclk_MASK,
                                     (0u != (pinsInBuf & UART_MASTER_CTS_SCLK_PIN_MASK)));
    #endif /* (UART_MASTER_CTS_SCLK_PIN) */

    #if (UART_MASTER_RTS_SS0_PIN)
        UART_MASTER_SET_HSIOM_SEL(UART_MASTER_RTS_SS0_HSIOM_REG,
                                       UART_MASTER_RTS_SS0_HSIOM_MASK,
                                       UART_MASTER_RTS_SS0_HSIOM_POS,
                                       hsiomSel[UART_MASTER_RTS_SS0_PIN_INDEX]);

        UART_MASTER_uart_rts_spi_ss0_SetDriveMode((uint8) pinsDm[UART_MASTER_RTS_SS0_PIN_INDEX]);

        UART_MASTER_SET_INP_DIS(UART_MASTER_uart_rts_spi_ss0_INP_DIS,
                                     UART_MASTER_uart_rts_spi_ss0_MASK,
                                     (0u != (pinsInBuf & UART_MASTER_RTS_SS0_PIN_MASK)));
    #endif /* (UART_MASTER_RTS_SS0_PIN) */
